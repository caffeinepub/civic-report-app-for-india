...
import { Search, Mail, Phone, MapPin, Award, User, Filter, Calendar, Star } from 'lucide-react';
import { useGetVolunteerDirectory, useGetMyVolunteerProfile, useIsAdmin } from '../hooks/useQueries';
...
export function VolunteerDirectory() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'joinDate' | 'location' | 'impactScore'>('impactScore');
  ...
  const filteredVolunteers = volunteers
    ?.filter(volunteer => 
      ...
      volunteer.principal.toString().toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      switch (sortBy) {
        case 'name':
        ...
        case 'joinDate':
          ...
          const aDate = a.approvalTimestamp ? Number(a.approvalTimestamp) : Number(a.applicationDate);
          const bDate = b.approvalTimestamp ? Number(b.approvalTimestamp) : Number(b.applicationDate);
          return bDate - aDate;
        case 'location':
    ...
    return (
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Loading Volunteer Directory...</h1>
          <p className="text-gray-600">Please wait while we fetch the verified volunteer information.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="text-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Verified Volunteer Directory</h1>
          <p className="text-gray-600">
            Meet our community of verified volunteers with complete profiles, photos, and contact information making a difference in civic engagement across India.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'name' | 'joinDate' | 'location' | 'impactScore')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-200">
          <div className="text-center">
            <h3 className="text-2xl font-bold text-blue-600">{volunteers?.length || 0}</h3>
          ...
          <div className="text-center">
            <h3 className="text-2xl font-bold text-purple-600">
              {volunteers?.reduce((sum, v) => sum + Number(v.impactScore), 0) || 0}
            </h3>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mt-8 text-center">
        ...
        <h3 className="text-lg font-semibold text-blue-900 mb-2">Want to become a verified volunteer?</h3>
        ...
        >
          {isLoggedInVolunteer ? 'Go to Volunteer Dashboard' : 'Apply to Become a Volunteer'}
        </button>
      </div>
    </div>
  );
...
function VolunteerCard({ volunteer, contactInfo, isAdmin, formatMobileNumber }: { 
  volunteer: any; 
  ...
  const formattedMobile = formatMobileNumber(contactInfo.mobile, volunteer.showFullMobile);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
      ...
      <div className="flex items-center space-x-2 text-xs text-gray-500 border-t border-gray-200 pt-3">
        ...
        <span>
          Volunteer since {new Date(volunteerSinceDate).toLocaleDateString('en-IN', {
            year: 'numeric',
            month: 'long'
          })}
        </span>
