...
export function AdminFeedbackManagement() {
  ...
  const { mutate: updateStatus, isPending: isUpdatingStatus } =
    useUpdateFeedbackStatus();
  const { mutate: respondToFeedback, isPending: isResponding } =
    useRespondToFeedback();
  ...
  const handleStatusChange = (feedbackId: string, newStatus: string) => {
    if (
      confirm(
        `Are you sure you want to mark this submission as "${newStatus}"?`,
      )
    ) {
  ...
  const inProgressSubmissions = filteredFeedback.filter(
    (f) => f.status === "In Progress",
  );
  const resolvedSubmissions = filteredFeedback.filter(
    (f) => f.status === "Resolved",
  );
    ...
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-2 sm:space-y-0">
        ...
        <div className="text-xs sm:text-sm text-gray-600">
          {filteredFeedback.length} total • {newSubmissions.length} new •{" "}
          {inProgressSubmissions.length} in progress •{" "}
      ...
      {!allFeedback || allFeedback.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm sm:text-base text-gray-600">
            No feedback or contact submissions have been received.
          </p>
        </div>
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-xs text-gray-600">
                  From: {selectedFeedback.contactInfo}
                </p>
              </div>
