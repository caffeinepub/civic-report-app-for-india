...
import {
  useAddConstituency,
  useAddMlaToConstituency,
  useAddMpToConstituency,
  useAddState,
  useAddUnionTerritory,
  ...
  useUpdateRepresentativeDetails,
  useUpdateState,
  useUpdateUnionTerritory,
...
export function AdminDirectory() {
  ...
  const { mutate: addConstituency, isPending: _isAddingConstituency } =
    useAddConstituency();
  ...
  const { mutate: _updateRep, isPending: _isUpdatingRep } =
    useUpdateRepresentative();
  const { mutate: deleteConstituency, isPending: _isDeletingConstituency } =
    useDeleteConstituency();
  const { mutate: deleteRep, isPending: _isDeletingRep } =
    useDeleteRepresentative();
  const { mutate: setPM, isPending: isSettingPM } = useSetPrimeMinister();
  const { mutate: exportDirectory, isPending: isExporting } =
    useExportDirectory();
  const { mutate: importDirectory, isPending: isImporting } =
    useImportDirectory();
  ...
  const validateForm = (): string | null => {
    ...
    if (activeForm === "state") {
      ...
      if (
        allStates.some(
          (s) =>
            s.name.toLowerCase() === formData.stateName.trim().toLowerCase(),
        )
      ) {
    ...
    if (activeForm === "lok-sabha" || activeForm === "vidhan-sabha") {
      ...
      if (activeForm === "lok-sabha") {
        const existingConstituency = selectedState.constituencies.find(
          (c) =>
            c.name.toLowerCase() ===
            formData.constituencyName.trim().toLowerCase(),
        );
      ...
      if (activeForm === "prime-minister") {
        ...
        if (formData.isUT) {
          await new Promise<void>((resolve, reject) => {
            addUT(
              {
                utName: formData.stateName.trim(),
                administrator: representative,
              },
              {
                onSuccess: () => {
                  toast.success(
                    `Union Territory "${formData.stateName}" added successfully!`,
                  );
                  resolve();
                },
                onError: (error) => {
                  ...
                  toast.error(
                    "Failed to add union territory. Please try again.",
                  );
                  reject(error);
                },
              },
          ...
          await new Promise<void>((resolve, reject) => {
            addState(
              { stateName: formData.stateName.trim(), cm: representative },
              {
                onSuccess: () => {
                  toast.success(
                    `State "${formData.stateName}" added successfully!`,
                  );
                  resolve();
                },
                onError: (error) => {
            ...
            addConstituency(
              ...
              },
              {
                ...
                onError: (error) => {
                  ...
                  toast.error(
                    "Failed to create constituency. Please try again.",
                  );
                  reject(error);
                },
              },
        ...
        await new Promise<void>((resolve, reject) => {
          addMp(
            ...
            },
            {
              onSuccess: () => {
                toast.success(
                  `MP "${formData.name}" added to ${formData.constituencyName} successfully! ✅`,
                );
                resolve();
              },
              onError: (error) => {
        ...
        await new Promise<void>((resolve, reject) => {
          addMla(
            ...
            },
            {
              onSuccess: () => {
                toast.success(
                  `MLA "${formData.name}" added to ${formData.constituencyName} successfully! ✅`,
                );
                resolve();
              },
              onError: (error) => {
    ...
    try {
      ...
      exportDirectory(undefined, {
        onSuccess: async (exportedDirectory) => {
          ...
          link.setAttribute(
            "download",
            `administrative-directory-${Date.now()}.csv`,
          );
  ...
  const convertDirectoryToCSV = async (dir: any): Promise<string> => {
    ...
    rows.push(
      "Level,Unit Name,Representative Name,Photo Path,Email,Twitter Handle,Political Party,Remarks,Last Updated",
    );
        ...
        if (constituency.mp) {
          rows.push(
            escapeCSV([
              ...
              new Date(
                Number(constituency.mp.lastUpdated) / 1000000,
              ).toISOString(),
            ]),
          );
        }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      importDirectory(newDirectory, {
        onSuccess: () => {
          toast.success(
            "Directory imported successfully with image paths retained!",
          );
        ...
        onError: (error) => {
          ...
          toast.error(
            "Failed to import directory. Please check the file format and try again.",
          );
          if (fileInputRef.current) {
            fileInputRef.current.value = "";
          }
  ...
  const parseCSVToDirectory = async (csvText: string): Promise<any> => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (photoPath?.trim()) {
        ...
        console.log(
          `Reusing existing image path for ${repName}: ${finalPhotoPath}`,
        );
      } else {
        // If no photoPath, this is a new entry requiring manual upload
        console.warn(
          `No image path found for ${repName}. Manual upload will be required.`,
        );
      }
      ...
      if (level === "PM") {
        ...
        let constituency = state.constituencies.find(
          (c: any) => c.name === constituencyName,
        );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
        ...
        <p className="text-sm sm:text-base text-gray-600 mb-2">
          Manage information for Prime Minister, 28 States, 8 Union Territories,
          543 Lok Sabha MPs, and 4,131 Vidhan Sabha MLAs
        </p>
        <p className="text-xs text-blue-600 bg-blue-50 p-2 rounded">
          <AlertCircle className="inline h-3 w-3 mr-1" />
          Export/Import retains image paths (photoPath) for efficient backup and
          restoration. Existing images are reused automatically during import.
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 border-t pt-4">
              ...
              <div className="flex items-center space-x-4 mb-4">
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, isUT: e.target.checked })
                    }
            ...
            {activeForm === "state" && (
              <div>
                ...
                >
                  {formData.isUT ? "Union Territory Name" : "State Name"}{" "}
              ...
              <>
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    <AlertCircle className="inline h-3 w-3 mr-1" />
                    Constituency will be created automatically if it doesn't
                    exist
                  </p>
                </div>
              </>
              ...
              <table className="min-w-full border-collapse">
                <thead>
                  <tr className="bg-gray-100 border-b-2 border-gray-300">
                    ...
                    <th className="text-center p-2 sm:p-3 text-xs sm:text-sm font-semibold text-gray-700 whitespace-nowrap">
                      Actions
                    </th>
                  </tr>
                </thead>
...
function PrimeMinisterTableRow({
  ...
  const [photoFile, setPhotoFile] = useState<File | null>(null);
  const [_photoPreview, setPhotoPreview] = useState<string>("");
    ...
    <tr className="border-b border-gray-200 bg-purple-50 hover:bg-purple-100">
      ...
      <td className="p-2 sm:p-3">
        {isEditing ? (
          <input
            ...
            onChange={(e) =>
              setEditData({ ...editData, politicalParty: e.target.value })
            }
            className="w-full px-2 py-1 border border-gray-300 rounded text-xs sm:text-sm"
          />
        ) : (
      ...
      <td className="p-2 sm:p-3">
        {isEditing ? (
          ...
          <span className="text-xs sm:text-sm text-gray-700 max-w-xs truncate block">
            {primeMinister?.remarks || "-"}
          </span>
        )}
      </td>
...
function StateTableRows({
  ...
  isExpanded,
  onToggle,
  expandedConstituencies: _expandedConstituencies,
  onToggleConstituency: _onToggleConstituency,
  viewMode,
  ...
  const handleSave = async () => {
    try {
      ...
      if (editData.isUT !== state.isUnionTerritory) {
        ...
        if (editData.isUT) {
          updateUT(
            { utName: state.name, updatedUT: updatedState },
            {
              onSuccess: async () => {
                await refetchDirectory();
                toast.success(
                  `Successfully moved "${editData.stateName}" from State to Union Territory!`,
                );
                setIsEditing(false);
                setPhotoFile(null);
                setPhotoPreview("");
          ...
          updateState(
            { stateName: state.name, updatedState },
            {
              onSuccess: async () => {
                await refetchDirectory();
                toast.success(
                  `Successfully moved "${editData.stateName}" from Union Territory to State!`,
                );
                setIsEditing(false);
                setPhotoFile(null);
                setPhotoPreview("");
  ...
  return (
    <>
        ...
        <td className="p-2 sm:p-3">
          {isEditing ? (
            <select
              ...
              onChange={(e) =>
                setEditData({ ...editData, isUT: e.target.value === "UT" })
              }
        ...
        <td className="p-2 sm:p-3">
          {isEditing ? (
            <input
              ...
              onChange={(e) =>
                setEditData({ ...editData, stateName: e.target.value })
              }
      ...
      {isExpanded && (
        <>
          {showLokSabha &&
            lokSabhaConstituencies.map((constituency, constIndex) => (
          ...
          {showVidhanSabha &&
            vidhanSabhaConstituencies.map((constituency, constIndex) => (
              <ConstituencyTableRow
...
function ConstituencyTableRow({
  ...
  type,
  viewMode: _viewMode,
  onDeleteConstituency: _onDeleteConstituency,
  onDeleteRepresentative,
  onPhotoClick,
}: {
  stateName: string;
...
function RepresentativeTableRow({
  ...
  const handleSave = async () => {
    try {
      ...
      if (editData.constituencyName !== constituencyName) {
        ...
        const oldConstituency = state?.constituencies.find(
        ...
        if (oldConstituency) {
          ...
          updateConstituency(
            ...
            },
            {
              onSuccess: () => {
                toast.success(
                  `${type === "Lok Sabha" ? "MP" : "MLA"} and constituency updated successfully!`,
                );
                setIsEditing(false);
                setPhotoFile(null);
                setPhotoPreview("");
              ...
              onError: (error) => {
                ...
                toast.error(
                  `Failed to update ${type === "Lok Sabha" ? "MP" : "MLA"} and constituency.`,
                );
              },
            },
          );
        ...
        updateRepDetails(
          ...
          },
          {
            onSuccess: () => {
              toast.success(
                `${type === "Lok Sabha" ? "MP" : "MLA"} updated successfully!`,
              );
              setIsEditing(false);
              setPhotoFile(null);
              setPhotoPreview("");
            ...
            onError: (error) => {
              ...
              toast.error(
                `Failed to update ${type === "Lok Sabha" ? "MP" : "MLA"}.`,
              );
            },
          },
        );
  ...
  const handleDelete = () => {
    if (
      window.confirm(
        `Are you sure you want to delete ${type === "Lok Sabha" ? "MP" : "MLA"} "${representative.name}"?`,
      )
    ) {
      onDeleteRepresentative(
        {
        ...
        },
        {
          onSuccess: () => {
            toast.success(
              `${type === "Lok Sabha" ? "MP" : "MLA"} "${representative.name}" deleted successfully!`,
            );
          },
          onError: (error: any) => {
            console.error("Error deleting representative:", error);
            toast.error(
              `Failed to delete ${type === "Lok Sabha" ? "MP" : "MLA"}. Please try again.`,
            );
          },
        },
      );
        ...
        <td className="p-2 sm:p-3">
          {isEditing ? (
            <input
              ...
              onChange={(e) =>
                setEditData({ ...editData, constituencyName: e.target.value })
              }
              className="w-full px-2 py-1 border border-gray-300 rounded text-xs sm:text-sm"
            />
          ) : (
