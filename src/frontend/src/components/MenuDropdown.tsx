...
import { Menu, ChevronDown, BarChart3, FileText, Code, Users, Lightbulb, X, Shield, LogIn, LogOut, TrendingUp, Settings, Map as MapIcon, UserCheck, Award, BookOpen, Building2, Building, MessageSquare } from 'lucide-react';
import { Link } from '@tanstack/react-router';
import { useGetAllReports, useIsAdmin, useGetMyVolunteerProfile, useGetMyNgoNpoProfile, useGetTotalUniqueVisitors } from '../hooks/useQueries';
...
export function MenuDropdown() {
  ...
  const { data: totalUniqueVisitors, refetch: refetchVisitors } = useGetTotalUniqueVisitors();
  const { identity, login, clear, loginStatus } = useInternetIdentity();
  const { data: isAdmin } = useIsAdmin();
  const { data: volunteerProfile, isLoading: isLoadingVolunteerProfile } = useGetMyVolunteerProfile();
  const { data: ngoNpoProfile, isLoading: isLoadingNgoNpoProfile } = useGetMyNgoNpoProfile();
  ...
  const contributorsByRegion = reports?.reduce((acc, report) => {
    if (report.username) {
      const region = 'India'; // Simplified for now, could be enhanced with location parsing
      if (!acc[region]) acc[region] = {};
      acc[region][report.username] = (acc[region][report.username] || 0) + 1;
    }
    return acc;
  }, {} as Record<string, Record<string, number>>) || {};

  const contributorsByCategory = reports?.reduce((acc, report) => {
    if (report.username) {
      const category = report.issueType;
      if (!acc[category]) acc[category] = {};
      acc[category][report.username] = (acc[category][report.username] || 0) + 1;
    }
    return acc;
  }, {} as Record<string, Record<string, number>>) || {};
  ...
  const menuSections = [
    {
      ...
      content: (
        <div className="space-y-4">
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Analytics (Live Data)</h4>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span>Total Reports:</span>
                <span className="font-medium text-blue-600">{totalReports}</span>
              </div>
              <div className="flex justify-between">
                <span>Resolution Rate:</span>
                <span className="font-medium text-green-600">{resolutionRate}%</span>
              </div>
              <div className="mt-3">
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Impact Metrics (Real-time)</h4>
            <div className="text-sm">
              <div className="flex justify-between">
                <span>Issues Resolved:</span>
                <span className="font-medium text-green-600">{resolvedReports}</span>
              </div>
              <div className="flex justify-between mt-1">
                <span>Active Reports:</span>
                <span className="font-medium text-orange-600">{totalReports - resolvedReports}</span>
              </div>
            </div>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Leaderboard (Live)</h4>
            <div className="text-sm text-gray-600">
              {Object.keys(contributorsByRegion).length > 0 || Object.keys(contributorsByCategory).length > 0 ? (
                <div className="space-y-2">
                  <div>
                    <p className="font-medium text-gray-700">Top Contributors:</p>
                    {Object.entries(contributorsByCategory).slice(0, 3).map(([category, contributors]) => {
                      const topContributor = Object.entries(contributors).sort(([,a], [,b]) => b - a)[0];
                      return topContributor ? (
                        <div key={category} className="text-xs">
                          <span className="font-medium">{topContributor[0]}</span> - {category} ({topContributor[1]} reports)
                        </div>
                      ) : null;
                    })}
                  </div>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Find Your Representatives</h4>
            <p className="text-sm text-gray-600 mb-3">
              Discover your PM, CM, MP, MLA, and local civic body representatives. Use location detection or search manually by state, district, and constituency.
            </p>
            <Link
              to="/know-your-neta"
        ...
        <div className="space-y-4">
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">
              {isVolunteerAuthenticated ? 'Volunteer Dashboard' : 'Become a Verified Volunteer'}
            </h4>
            <p className="text-sm text-gray-600 mb-3">
              ...
              {isVolunteerAuthenticated ? (
                  ...
                  {isApprovedVolunteer && (
                    <div className="bg-green-50 border border-green-200 rounded p-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs text-green-800 font-medium">Verified Volunteer</span>
                      </div>
                    </div>
                  )}
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={loginStatus === 'logging-in' || isLoadingVolunteerProfile}
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Volunteer Directory</h4>
            <p className="text-sm text-gray-600 mb-3">
              Browse our community of verified volunteers and their contributions.
            </p>
            <Link
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Volunteer Benefits</h4>
            <div className="space-y-1 text-sm text-gray-600">
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">
              {isNgoNpoAuthenticated ? 'NGO/NPO Dashboard' : 'Register Your Organization'}
            </h4>
            <p className="text-sm text-gray-600 mb-3">
              ...
              {isNgoNpoAuthenticated ? (
                  ...
                  {isApprovedNgoNpo && (
                    <div className="bg-teal-50 border border-teal-200 rounded p-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs text-teal-800 font-medium">Verified Organization</span>
                      </div>
                    </div>
                  )}
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={loginStatus === 'logging-in' || isLoadingNgoNpoProfile}
                    className="block w-full bg-blue-600 text-white py-2 px-3 rounded text-center hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                  >
                    <div className="flex items-center justify-center space-x-2">
                      <LogIn className="h-4 w-4" />
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">NGO/NPO Directory</h4>
            <p className="text-sm text-gray-600 mb-3">
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Organization Benefits</h4>
            <div className="space-y-1 text-sm text-gray-600">
      ...
      content: (
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mb-3">
              View our feature roadmap and development progress. See what's implemented, under consideration, and planned for the future.
            </p>
            <Link
              to="/roadmap"
              className="block w-full bg-purple-600 text-white py-2 px-3 rounded text-center hover:bg-purple-700 transition-colors text-sm font-medium"
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3 text-sm">
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Data Privacy & Usage Policy</h4>
            <p className="text-gray-600">Your data is stored securely on the Internet Computer blockchain. We do not share personal information with third parties.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">User Responsibilities</h4>
            <p className="text-gray-600">Users must provide accurate information and use the platform responsibly for legitimate civic reporting.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Liability Disclaimers</h4>
            <p className="text-gray-600">This platform is for civic awareness. We do not guarantee official action on reports.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Community Guidelines</h4>
            <p className="text-gray-600">Maintain respectful communication and report genuine civic issues only.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Volunteer & NGO/NPO Program</h4>
            <p className="text-gray-600">Verified volunteers and organizations must maintain high standards of conduct and provide accurate information in their applications.</p>
          </div>
        </div>
      )
    },
      ...
      content: (
        <div className="space-y-3 text-sm">
          <div>
            ...
            <p className="text-gray-600">A decentralized civic reporting platform built for Indian citizens to report and track infrastructure issues.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Tech Stack</h4>
            <p className="text-gray-600">Built on ICP Blockchain with Caffeine AI for enhanced user experience and data integrity.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">How It Works</h4>
            <p className="text-gray-600">Report issues with photos, get certificates, track status updates, and contribute to community improvement.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Version History (Live)</h4>
            <div className="space-y-1">
              <p className="text-gray-600">Last updated: <span className="font-medium">{currentDate}</span></p>
              <p className="text-gray-600">Current version: <span className="font-medium">v3.2.0</span></p>
              <p className="text-gray-600">Total reports processed: <span className="font-medium text-blue-600">{totalReports}</span></p>
              <p className="text-gray-600">Total unique visitors: <span className="font-medium text-purple-600">{totalUniqueVisitors ?? 0}</span></p>
              <p className="text-gray-600">Platform uptime: <span className="font-medium text-green-600">99.9%</span></p>
            </div>
          </div>
        </div>
      )
      ...
      content: (
        <div className="space-y-3 text-sm">
          <div>
            ...
            <p className="text-gray-600">Contribute code and features to improve the platform. Contact us for collaboration opportunities.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">For Citizens</h4>
            <p className="text-gray-600">Spread awareness about civic issues, share the platform, and provide suggestions for improvement.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Funding</h4>
            <p className="text-gray-600">Support platform maintenance through donations. Every contribution helps keep the service running and accessible to all.</p>
            <a 
          ...
          <div>
            ...
            <p className="text-gray-600 mb-2">Submit feature requests and bug reports to help us improve the platform for everyone.</p>
            <button
      ...
      content: (
        <div className="space-y-3 text-sm">
          <div>
            ...
            <p className="text-gray-600">Latest updates: NGO/NPO onboarding system with directory and dashboard, Know Your Neta directory for finding representatives, separate admin and volunteer roles with strict access control, optimized volunteer login flow, enhanced volunteer authentication, and volunteer badge system for community recognition.</p>
          </div>
          <div>
            <h4 className="font-semibold text-gray-900 mb-1">Tips & Best Practices</h4>
            <div className="space-y-1 text-gray-600">
              <p><strong>Reporting:</strong> Take clear photos, provide accurate location, add detailed descriptions.</p>
              <p><strong>Sharing:</strong> Tag relevant ministers and officials when sharing on social media.</p>
              <p><strong>Certificates:</strong> Use certificates as proof of civic engagement and community contribution.</p>
              <p><strong>Status Updates:</strong> Anyone can update report status with proof photos.</p>
              <p><strong>Volunteers:</strong> Apply for verified status to gain credibility and recognition.</p>
              <p><strong>NGOs/NPOs:</strong> Register your organization for visibility and credibility.</p>
              <p><strong>Know Your Neta:</strong> Use the directory to find and contact your representatives.</p>
            </div>
          </div>
        </div>
      )
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between">
                <span>Principal:</span>
                <span className="font-mono text-xs text-gray-600 truncate max-w-32" title={identity.getPrincipal().toString()}>
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Admin Management Panel</h4>
            <p className="text-xs text-gray-600 mb-2">
              Manage admin access, permissions, volunteer applications, and NGO/NPO registrations.
            </p>
        ...
        <div className="space-y-4">
            ...
            {identity ? (
              <div className="space-y-3">
                <div className="bg-yellow-50 border border-yellow-200 rounded p-3">
                  ...
                  <p className="text-xs text-gray-600 font-mono truncate" title={identity.getPrincipal().toString()}>
                    {identity.getPrincipal().toString().slice(0, 20)}...
                  </p>
                </div>
                <p className="text-sm text-gray-600">
                  You are logged in but do not have admin privileges. You can log out and try a different Internet Identity if needed.
                </p>
              </div>
            ) : (
              <p className="text-sm text-gray-600 mb-3">
          ...
          <div className="space-y-2">
            {identity ? (
              ...
              >
                <LogIn className="h-4 w-4" />
                <span>{loginStatus === 'logging-in' ? 'Signing in...' : 'Sign in with Internet Identity'}</span>
              </button>
            )}
          </div>
          
          <div className="text-xs text-gray-500">
            <p><strong>Note:</strong> Only authorized administrators can access admin features. Contact the platform administrator if you need admin access.</p>
          </div>
        </div>
      )
    }
              ...
              <div key={section.id} className="menu-section">
                <button
                  ...
                  className={`menu-section-header ${
                    activeSection === section.id ? 'menu-section-header-active' : ''
                  }`}
                >
                  <div className="flex items-center space-x-2">
                    {section.icon}
