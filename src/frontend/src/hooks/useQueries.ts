...
export function useGetRecentReports(count = 5) {
  const { actor, isFetching } = useActor();
...
export function useSubmitReport() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const finalAddress =
        address && address.trim() !== "" ? address : coordinates;
...
export function useUpdateReportStatus() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return actor.updateReportStatus(
        reportId,
        status,
        proofPhotoPath,
        reporterName,
...
export function useCreateFeature() {
  ...
  return useMutation({
    mutationFn: async ({
      sectionId,
      featureData,
    }: { sectionId: string; featureData: RoadmapFeature }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useUpdateFeature() {
  ...
  return useMutation({
    mutationFn: async ({
      featureId,
      featureData,
    }: { featureId: string; featureData: RoadmapFeature }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useMoveFeature() {
  ...
  return useMutation({
    mutationFn: async ({
      featureId,
      newSectionId,
    }: { featureId: string; newSectionId: string }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useRejectVolunteer() {
  ...
  return useMutation({
    mutationFn: async ({
      volunteerId,
      rejectionNote,
    }: { volunteerId: string; rejectionNote: string }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useUpdateVolunteerPrivacy() {
  ...
  return useMutation({
    mutationFn: async ({
      volunteerId,
      showFullMobile,
    }: { volunteerId: string; showFullMobile: boolean }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useSubmitVolunteerProfileEdit() {
  ...
  return useMutation({
    mutationFn: async ({
      volunteerId,
      updates,
    }: { volunteerId: string; updates: VolunteerProfileUpdate }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useRejectVolunteerProfileEdit() {
  ...
  return useMutation({
    mutationFn: async ({
      editId,
      rejectionNote,
    }: { editId: string; rejectionNote: string }) => {
      if (!actor) throw new Error("Actor not available");
    ...
    queryFn: async () => {
      if (!allReports || !volunteerName) {
        return {
          reportsSubmitted: 0,
          statusUpdates: 0,
          impactScore: volunteerProfile?.impactScore
            ? Number(volunteerProfile.impactScore)
            : 0,
        };
      }

      const volunteerReports = allReports.filter(
        (report) =>
          report.username === volunteerName ||
          report.reporterName === volunteerName,
      );

      const reportsSubmitted = volunteerReports.filter(
        (r) => r.username === volunteerName,
      ).length;
      const statusUpdates = volunteerReports.filter(
        (r) => r.reporterName === volunteerName && r.status === "Resolved",
      ).length;
      const impactScore = volunteerProfile?.impactScore
        ? Number(volunteerProfile.impactScore)
        : 0;
      ...
      try {
        ...
        if (currentUserPrincipal) {
          ...
          if (userApplication) {
            return {
              status:
                userApplication.status === ApprovalStatus.approved
                  ? "Approved"
                  : userApplication.status === ApprovalStatus.rejected
                    ? "Rejected"
                    : "Pending",
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const applications = await actor.listApprovals();
        const volunteerApplication = applications.find(
          (app) => app.principal.toString() === principal.toString(),
        );
  ...
  return useQuery<Constituency[]>({
    ...
    queryFn: async () => {
      ...
      return constituencies.filter(
        (constituency) =>
          constituency.mp !== null && constituency.mp !== undefined,
      );
    },
...
export function useAddState() {
  ...
  return useMutation({
    mutationFn: async ({
      stateName,
      cm,
    }: { stateName: string; cm: Representative | null }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useAddUnionTerritory() {
  ...
  return useMutation({
    mutationFn: async ({
      utName,
      administrator,
    }: { utName: string; administrator: Representative | null }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useAddMpToConstituency() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      constituencyName,
      mp,
    }: { stateName: string; constituencyName: string; mp: Representative }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useUpdateRepresentative() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateRepresentative(
        stateName,
        constituencyName,
        repType,
        representative,
...
export function useUpdateState() {
  ...
  return useMutation({
    mutationFn: async ({
      stateName,
      updatedState,
    }: { stateName: string; updatedState: State }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useUpdateConstituency() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateConstituency(
        stateName,
        constituencyName,
        updatedConstituency,
      );
...
export function useUpdateRepresentativeDetails() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateRepresentativeDetails(
        stateName,
        constituencyName,
        repType,
        repName,
...
export function useRejectNgoNpo() {
  ...
  return useMutation({
    mutationFn: async ({
      ngoNpoId,
      rejectionNote,
    }: { ngoNpoId: string; rejectionNote: string }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useUpdateNgoNpoPrivacy() {
  ...
  return useMutation({
    mutationFn: async ({
      ngoNpoId,
      showContactInfo,
    }: { ngoNpoId: string; showContactInfo: boolean }) => {
      if (!actor) throw new Error("Actor not available");
