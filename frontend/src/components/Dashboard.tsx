...
import { useGetInitialReports, useGetNextReports, useGetTotalReportCount, useGetVolunteerDirectory } from '../hooks/useQueries';
import { ReportCard } from './ReportCard';
import { Loader2, Filter, X, Home, Map, TrendingUp, CheckCircle, AlertCircle, FileText, ChevronDown } from 'lucide-react';
import { useLanguage } from '../contexts/LanguageContext';

export function Dashboard() {
  const { t } = useLanguage();
  const { data: initialReports, isLoading: isLoadingInitial } = useGetInitialReports();
  ...
  useEffect(() => {
    if (initialReports) {
      ...
      const sortedReports = [...initialReports].sort((a, b) => Number(b.timestamp) - Number(a.timestamp));
  ...
  const loadNextBatch = async () => {
    if (isLoadingMore || !totalCount || currentOffset >= Number(totalCount)) return;
    
    setIsLoadingMore(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_CANISTER_URL}/getNextReports?offset=${currentOffset}&count=50`);
  ...
  const filteredReports = React.useMemo(() => {
    ...
    if (selectedIssueType !== 'all') {
      filtered = filtered.filter((report: any) => report.issueType === selectedIssueType);
    }

    if (selectedStatus !== 'all') {
      filtered = filtered.filter((report: any) => report.status.toLowerCase() === selectedStatus);
    }

    if (selectedVolunteer !== 'all') {
      filtered = filtered.filter((report: any) => {
        const volunteer = volunteerDirectory?.find(v => v.id === selectedVolunteer);
        return volunteer && report.username === volunteer.name;
      });
    }

    return filtered;
  }, [allReports, selectedIssueType, selectedStatus, selectedVolunteer, volunteerDirectory]);

  const stats = React.useMemo(() => {
    if (!allReports) return { total: 0, open: 0, resolved: 0, resolutionRate: 0 };

    const total = allReports.length;
    const resolved = allReports.filter((r: any) => r.status.toLowerCase() === 'resolved').length;
  ...
  const hasActiveFilters = selectedIssueType !== 'all' || selectedStatus !== 'all' || selectedVolunteer !== 'all';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 max-w-7xl py-4 sm:py-8">
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="text-center sm:text-left">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-1 sm:mb-2 leading-tight">Dashboard</h1>
              <p className="text-sm sm:text-base lg:text-lg text-gray-600 leading-relaxed">Comprehensive view of all civic reports (Live Data)</p>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-3 sm:p-4">
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <div>
                ...
                <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.resolutionRate}%</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Load More Button */}
        {!hasActiveFilters && totalCount && currentOffset < Number(totalCount) && (
            ...
            >
              {isLoadingMore ? (
                ...
                <>
                  ...
                  <span>Load Next 50 Reports ({currentOffset}/{Number(totalCount)})</span>
                </>
              )}
            </button>
          </div>
