...
import { Loader2, ArrowLeft, CheckCircle, X, Home } from 'lucide-react';
import { useLanguage } from '../contexts/LanguageContext';

export function ReportPage() {
  const { t } = useLanguage();
  const { reportId } = useParams({ from: '/report/$reportId' });
  const search = useSearch({ from: '/report/$reportId' }) as { submitted?: string };
  ...
  useEffect(() => {
    if (search.submitted === 'true') {
      ...
      sessionStorage.setItem('reportSubmissionSuccess', JSON.stringify({
        reportId: reportId,
        timestamp: Date.now()
      }));
    }
  ...
  useEffect(() => {
    ...
    if (storedSuccess) {
      try {
        ...
        if (successData.reportId === reportId && Date.now() - successData.timestamp < 5 * 60 * 1000) {
          setShowSuccessMessage(true);
        } else if (successData.reportId !== reportId || Date.now() - successData.timestamp >= 5 * 60 * 1000) {
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 sm:p-6 shadow-md sticky top-20 z-40">
          <div className="flex items-start justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-lg sm:text-xl font-bold text-green-800 mb-2">Report Submitted Successfully!</h3>
                <p className="text-sm sm:text-base text-green-700 mb-2">
                  Your civic report has been saved and is now ready for sharing and certificate generation.
                </p>
                <p className="text-sm sm:text-base text-green-700">
                  <strong>Report ID:</strong> <span className="font-mono bg-green-100 px-2 py-1 rounded text-xs sm:text-sm">{reportId}</span>
                </p>
                <p className="text-xs sm:text-sm text-green-600 mt-2">
                  You can now generate your certificate by clicking the "Certificate" button below, or share your report on social media.
                </p>
                <p className="text-xs text-green-500 mt-2">
                  This message will remain visible until you navigate away or dismiss it.
                </p>
              </div>
            </div>
            <button
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
        <div className="space-y-4">
          ...
          <div className="text-center sm:text-left">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Report Details</h1>
            <p className="text-sm sm:text-base text-gray-600">View and manage your civic report</p>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h4 className="font-medium text-blue-900 mb-2">Share Your Report</h4>
            <p className="text-blue-700 text-sm mb-3">
              Help raise awareness by sharing your report on social media and tagging relevant officials.
            </p>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <h4 className="font-medium text-green-900 mb-2">Download Certificate</h4>
            <p className="text-green-700 text-sm mb-3">
              Generate and download your civic engagement certificate as proof of your contribution.
            </p>
