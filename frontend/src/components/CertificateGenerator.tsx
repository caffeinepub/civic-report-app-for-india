...
import { Download, Award, Check, User } from 'lucide-react';
import { Report } from '../backend';
import { useFileUrl } from '../blob-storage/FileStorage';
import { useLanguage } from '../contexts/LanguageContext';
import { useGetCurrentLogo, useGetVolunteerDirectory } from '../hooks/useQueries';
...
export function CertificateGenerator({ report, imageUrl, onCertificateGenerated }: CertificateGeneratorProps) {
  const { t } = useLanguage();
  ...
  const availableLeaders: LeaderOption[] = React.useMemo(() => {
    ...
    return leaders;
  }, [report, pmPhotoUrl, cmPhotoUrl, mpPhotoUrl, mlaImageUrl, civicBodyPhotoUrl]);
  ...
  const formatComprehensiveAddress = (locationData: LocationData): string[] => {
    ...
    else if (locationData.city_district) cityParts.push(locationData.city_district);
    
    if (locationData.county && !cityParts.includes(locationData.county)) {
      cityParts.push(locationData.county);
    } else if (locationData.state_district && !cityParts.includes(locationData.state_district)) {
  ...
  const loadImageFromUrl = async (src: string, timeout: number = 15000): Promise<HTMLImageElement | null> => {
  ...
  const drawLogoFromDataUrl = async (ctx: CanvasRenderingContext2D, dataUrl: string, x: number, y: number, size: number): Promise<void> => {
    try {
  ...
  const drawPlaceholderLogo = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number) => {
    ctx.save();
  ...
  const drawVolunteerBadge = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number = 16) => {
    ctx.save();
  ...
  const generateCertificate = async (leadersToShow: LeaderOption[]) => {
    ...
    try {
      ...
      if (currentLogo && (currentLogo as string).trim() !== '') {
        await drawLogoFromDataUrl(ctx, currentLogo as string, margin, logoY, 80);
      } else {
      ...
      ctx.font = '24px Arial';
      ctx.fillText(t('certificate.titleHindi'), margin, certificateHeaderY + 40);
      ...
      if (report.customAddress) {
        ...
        const locationData = await fetchLocationData(report.location.latitude, report.location.longitude);
        locationDisplay = formatComprehensiveAddress(locationData);
      }

      const imageLoadPromises = [
        loadImageFromUrl(imageUrl, 10000)
      ];

      // Load selected leader images
      const leaderImagePromises = leadersToShow.map(leader => 
        leader.photoUrl ? loadImageFromUrl(leader.photoUrl, 15000) : Promise.resolve(null)
      );

      const [reportImg, ...leaderImages] = await Promise.all([...imageLoadPromises, ...leaderImagePromises]);
      ...
      drawImageCover(ctx, reportImg, rightColumnX, issuePhotoY + 10, issuePhotoWidth, issuePhotoHeight, 'Report\nImage');
      ...
      ctx.fillText(`${t('certificate.reportId')} ${report.id}`, margin, detailsY);
      detailsY += 40;
      
      const date = new Date(Number(report.timestamp) / 1000000);
      ctx.fillText(`${t('certificate.date')} ${date.toLocaleDateString('en-IN')}`, margin, detailsY);
      detailsY += 35;
      ctx.fillText(`${t('certificate.time')} ${date.toLocaleTimeString('en-IN')}`, margin, detailsY);
      detailsY += 40;
      
      ctx.fillText(`${t('certificate.issueType')} ${report.issueType}`, margin, detailsY);
      detailsY += 40;
      ...
      ctx.fillText(`${t('certificate.coordinates')} ${coordinatesText}`, margin, detailsY);
      detailsY += 50;
      ...
      const totalMinisterWidth = leadersToShow.length * ministerPhotoWidth + (leadersToShow.length - 1) * ministerSpacing;
      const ministerStartX = rightColumnX + (issuePhotoWidth - totalMinisterWidth) / 2;
      const ministerPhotoY = ministersY + 40;

      // Draw selected leaders with object-fit: cover logic
      leadersToShow.forEach((leader, index) => {
        const leaderX = ministerStartX + index * (ministerPhotoWidth + ministerSpacing);
        const leaderImg = leaderImages[index];
        
        // Use object-fit: cover logic for leader photos
        drawImageCover(ctx, leaderImg, leaderX, ministerPhotoY, ministerPhotoWidth, ministerPhotoHeight, `${leader.designation}\nPhoto`);
        
        ctx.fillStyle = '#1f2937';
        ctx.font = 'bold 16px Arial';
        ctx.textAlign = 'center';
        ctx.fillText(leader.designation.toUpperCase(), leaderX + ministerPhotoWidth / 2, ministerPhotoY + ministerPhotoHeight + 20);
        ctx.font = '14px Arial';
        ctx.fillText(leader.name, leaderX + ministerPhotoWidth / 2, ministerPhotoY + ministerPhotoHeight + 40);
        
        // Display constituency if available (for MP)
        if (leader.constituency) {
          ctx.font = '12px Arial';
          ctx.fillStyle = '#6b7280';
          ctx.fillText(leader.constituency, leaderX + ministerPhotoWidth / 2, ministerPhotoY + ministerPhotoHeight + 58);
        }
      });
        ...
        if (qrImg) {
          ...
          if (ctx.measureText(urlText).width > maxUrlWidth) {
            ...
            for (const part of urlParts) {
              ...
              if (ctx.measureText(testLine).width > maxUrlWidth && currentLine) {
      ...
      ctx.fillText(t('certificate.initiative'), canvas.width / 2, statementY + 30);
      ...
      if (report.status.toLowerCase() === 'resolved' && report.reporterName) {
        ...
        if (report.completionNotes) {
          ctx.fillText(`Notes: ${report.completionNotes}`, margin, resolutionY + 80);
        }
      }
  ...
  const handleDownload = async () => {
    ...
    const defaultSelection = availableLeaders.slice(0, Math.min(3, availableLeaders.length)).map(l => l.id);
    setSelectedLeaders(defaultSelection);
  };

  const handleConfirmSelection = async () => {
    const leadersToShow = availableLeaders.filter(l => selectedLeaders.includes(l.id));
      ...
      {showLeaderSelection && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Select Leaders for Certificate</h3>
            <p className="text-sm text-gray-600 mb-4">Choose up to 3 responsible leaders to display on the certificate:</p>
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="text-left">
                      ...
                      {leader.constituency && (
                        <p className="text-xs text-gray-400">{leader.constituency}</p>
                      )}
                    </div>
                  </div>
                  {selectedLeaders.includes(leader.id) && (
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                disabled={selectedLeaders.length === 0 || selectedLeaders.length > 3}
